...
//validate the message input (contact up form)
// required && >75 && <250 
function validateMessage($message)
{
    $arr = [];
    if (requiredIsEmpty($message)) {
        $arr[] = "Message is required";
    } elseif (minLength($message, 75)) {
        $arr[] = "Message must be more than 75 chars";
    } elseif (maxLength($message, 250)) {
        $arr[] = "Message must be less than 250 chars";
    }
...
//validate the post title (create post form)
// required && >3 && <50 
function validatePostTitle($title)
{
    $arr = [];
    if (requiredIsEmpty($title)) {
        $arr[] = "Name is required";
    } elseif (minLength($title, 3)) {
        $arr[] = "Name must be more than 3 chars";
    } elseif (maxLength($title, 50)) {
        $arr[] = "Name must be less than 25 chars";
    }
...
//validate the post body (create post form)
// required && >75 && <250 
function validatePostBody($body)
{
    $arr = [];
    if (requiredIsEmpty($body)) {
        $arr[] = "Post Body is required";
    } elseif (minLength($body, 75)) {
        $arr[] = "Post Body must be more than 75 chars";
    } elseif (maxLength($body, 250)) {
        $arr[] = "Post Body must be less than 250 chars";
    }
